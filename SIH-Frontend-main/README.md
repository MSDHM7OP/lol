...
⚠️ **IMPORTANT:** This project uses API keys for OpenAI and Google Gemini services.

### For Production Deployment:
1. **Never commit API keys** to version control
2. **Store API keys in backend environment variables only**
3. **Frontend calls backend proxy endpoints** (see `BACKEND_PROXY_SETUP.md`)
4. **.env files are in `.gitignore`** - never commit them

### Security Documentation:
- **[SECURITY_AUDIT_REPORT.md](./SECURITY_AUDIT_REPORT.md)** - Complete security audit
- **[BACKEND_PROXY_SETUP.md](./BACKEND_PROXY_SETUP.md)** - Backend implementation guide
- **[DEPLOYMENT_GUIDE.md](./DEPLOYMENT_GUIDE.md)** - Deployment instructions for Render, Railway, Vercel, Netlify

### Quick Start (Secure):
1. Copy `.env.example` to `.env.local`
2. **Never add real API keys to .env**
3. Implement backend proxies (see guides above)
